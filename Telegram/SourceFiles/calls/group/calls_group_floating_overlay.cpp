...
void FloatingOverlay::setupUI() {
	auto *title = new Ui::FlatLabel(this, st::callButtonLabel);
	title->setText(QStringLiteral("Chat"));
	title->move(10, 10);

	auto *closeBtn = new Ui::IconButton(this, st::callAnswer.button);
	closeBtn->move(width() - 30, 5);
	closeBtn->setClickedCallback([this] { hide(); });

	auto *passthroughBtn = new Ui::IconButton(this, st::callAnswer.button);
	passthroughBtn->move(width() - 55, 5);
	passthroughBtn->setClickedCallback([this] { togglePassthrough(); });
